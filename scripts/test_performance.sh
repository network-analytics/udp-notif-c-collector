...
do 
  ...
  $UNYTE_COLLECTOR/$C_SENDER $DEST_IP $CS_PORT $CS_MESSAGES $CS_GEN_ID $CS_MSG_TYPE $CS_MTU $CS_MSG_SIZE &
  taskset -a -p 0x000000f0 $!
  # $UNYTE_COLLECTOR/$C_SENDER $DEST_IP $CS_PORT $((CS_MESSAGES / 2)) $((CS_MESSAGES / 2)) $CS_MSG_TYPE $CS_MTU $CS_MSG_SIZE &
  # taskset -a -p 0x000000f0 $!
  sleep 7
  $UNYTE_COLLECTOR/$C_SENDER $DEST_IP $CS_PORT $C_VLEN 49993648 $CS_MSG_TYPE $CS_MTU $CS_MSG_SIZE &
  taskset -a -p 0x000000f0 $!
