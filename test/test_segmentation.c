...
void test_segment_lists()
{
  ...
  struct message_segment_list_cell *seglist = create_message_segment_list(NULL, gid, mid); //Pool to null does not work!
  printf("Seglist @ %p\n", seglist);

  printf("Clearing\n");
  clear_msl(NULL, seglist);
  printf("Done %p\n", seglist);

  seglist = create_message_segment_list(NULL, gid, mid);

  //Add a new cell that is last in an empty list, seqnum 0
  int cont = 1;
  printf("Inserting into empty list\n");
  int val = insert_into_msl(NULL, seglist, 0, 1, 1, &cont);
...
void test_segment_buffers()
{
  ...
  for (i = 0; i < 10; i++)
  {
    ...
    *val = i + 1;
    insert_segment(NULL, buf, 1, 2, i, 0, 1, val);
    val = malloc(sizeof(int));
    *val = i;
    *val = *val * *val;
    insert_segment(NULL, buf, 2, 1, i, 0, 1, val);
  }
  ...
  clear_segment_list(NULL, buf, 1, 2);
  printf("Cleared list from 1,2\n");

  clear_segment_list(NULL, buf, 2, 1);
  ...
  int resclear = clear_buffer(NULL, buf);
